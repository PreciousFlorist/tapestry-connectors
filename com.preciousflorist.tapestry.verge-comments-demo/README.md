...
Version 14 removes the injected body byline and uses a native annotation ("by Author"). This provides compact metadata above the publisher header, rather than the Blog layout\'s byline below the headline. Exact Blog placement requires article layout and would change comment presentation. Publisher names, icons, native headlines, and comment identities remain intact.

Asset audit: preserve source HTML and inline image alt text; accept untyped media enclosures with recognizable media extensions; add Media RSS group and thumbnail fallbacks; deduplicate explicit attachment URLs; retain supplied dimensions and descriptions. Non-media enclosures remain excluded. The public feed and fallback favicon returned HTTP 200. Fixtures cover titles, bylines, RSS full content, Atom XHTML, icon precedence, links, inline media, enclosure fallback, thumbnails, audio, and nested comment identities. Native typography, embedded players, and private subscriber-feed assets require checking in Tapestry. No arbitrary CSS or unsupported small tags are used to approximate native text sizing.
